...
namespace defferedPassFunc {
	...
	namespace gBufferSubPass {
		...
		uint32_t amountOfLoadedAlbedos = 0;//hiba a pensar en como lidiar con unloading de imageViews, pero nada del c�digo lidia con eso as� q ps as� lo dejo y cuando lidie con unloading lo hago todo junto 
		...
		VkBuffer createMeshModelBuffer() {
			//llenar el struct (todos tienen la misma informaci�n)
			...
			returnBufferCI.size = ASG_MAX_MESH_MODEL_MATRICES * 64;//TODO 3000 arbitrario, 64 es el tama�o de una mat4
			returnBufferCI.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
			returnBufferCI.usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;

			//mandarlo a la funci�n (lo mismo)
		...
		void createPipeline(asgRenderSubPass* gBufferSubPass, VkRenderPass renderPass, uint32_t subPassIndex) {
			...
			VkPipelineColorBlendAttachmentState blendAttachmentState{};//para a�adir transparencia, solo adivinas los enums basado en como quieres que los combine
			...
			graphicsPipelineCI.basePipelineHandle = VK_NULL_HANDLE;//opcional, es para crear subpipelines que es m�s rapido que varias pipelines
		...
		void initializeDescriptorSets(asgRenderSubPass *gBufferSubPass) {
			...
			region.bufferImageHeight = 0;//podr�a ser que en el bufer existieran padding bytes entre los renglones, pero no padding, as� que 0 en este y rowLength
			...
			//le metemos los datos est�ticos
			for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
				...
				currMeshModelMatricesBI.range = ASG_MAX_MESH_MODEL_MATRICES * 64;//64 es el tama�o de una mat4, 50 es arbitrario
...
asgRenderPass createDefferedRenderingPass(asgSwapChain& sc) {
	...
	//creamos subpass dependencies/*describe dependencias (especifica memory y execution dependencies entre subpasses) para q el driver se encargue de eso (es optimo) //tenemos 3 subpasses??, la que creamos, la operaci�n antes y la operaci�n despu�s, vulkan tiene built-in dependencies que lidian con ellas pero hay que sincronizar la de la operaci�n despu�s??*/
	VkSubpassDependency beginToFirstSubPass{};
	beginToFirstSubPass.srcSubpass = VK_SUBPASS_EXTERNAL; // de quien dependo (indice) VK_SUBPASS_EXTERNAL se refiere a la operaci�n antes o despu�s
	beginToFirstSubPass.dstSubpass = 0;//quien soy yo (indice)
	beginToFirstSubPass.srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;//esperaremos a esta operaci�n????
	beginToFirstSubPass.srcAccessMask = 0;
	beginToFirstSubPass.dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;//esta operaci�n ser� la que espere???
	beginToFirstSubPass.dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;//espec�ficamente esperaremos hasta que acabe y luego escribiremos???

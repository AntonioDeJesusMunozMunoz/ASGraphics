...
VkSurfaceKHR windowSurface;//lo usa una funci�n de la swapchain y getSelectedQueueFamilies(osea main), ademas de momento la crea en asInit y existe la posiblidad de que la necesite yo al usarlo(o no)
GLFWwindow* ventana;
...
queueFamilyIndices getSelectedQueueFamilies(VkPhysicalDevice device) {
	...
	//checamos bitwise si tiene queue de graficos
...
uint32_t findRigthMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags memoryProperties) {
	...
	for (uint32_t i = 0; i < physicalDeviceMemoryProperties.memoryTypeCount; i++) {
		if (((1 << i) & typeFilter)  // 1 << i: el bit en la posici�n i le ponemos valor 1 y checamos si es el que buscamos, literal es solo bitwise curr == wanted
...
void transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
	...
	barrier.oldLayout = oldLayout;//este podr�a ser VK_IMAGE_LAYOUT_UNDEFINED si no te importa el contenido existente en la imagen
	...
	if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {//estos atributos de la barrera dependen de la transici�n
	...
	vkCmdPipelineBarrier(commandBuffer,
		...
		0,//la �nica otra opci�n es VK_DEPENDENCY_BY_REGION_BIT que permite leer de las regiones donde ya se haya escrito
...
VkCommandBuffer createSingleUseCmdBuffer() {
	/*Aloja E inicia un command buffer con ONE_TIME_SUBMIT flag*/
...
void createMemoryIndependentBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags wantedMemoryProperties, VkBuffer* buffer, VkDeviceMemory* bufferMemory, VkSharingMode sharingMode, uint32_t queueAmount, uint32_t* queueFamilyIndices) {
	//llenado
	...
	bufferCI.sharingMode = sharingMode;//lo usar� la transfer y graphics queue, all usar diferente queue para staging buffer puede que vea beneficios en velocidad(m�s queues es similar a m�s threads, m�s r�pido, pero solo si no hay demasiada espera para sincronizarlas), tambi�n SHARING_MODE_CONCURRENT puede ser m�s lento que SHARING_MODE_EXCLUSIVE, el industry standard es una queue que solo se dedica a transfers host->driver
	bufferCI.size = size;
	...
	vkBindBufferMemory(logicalDevice, *buffer, *bufferMemory, 0);//si el offset no fuera 0, deber�a ser divisible entre memoryRequirements.alignment
...
void copyBuffer(VkBuffer srcBuffer, VkDeviceSize srcOffset, VkBuffer dstBuffer, VkDeviceSize dstOffset, VkDeviceSize size) {
	...
	beginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;//le decimos que solo se usar� una vez
	...
	//vkFreeCommandBuffers(logicalDevice, commandPool, 1, &transferCommandBuffer);//quiz� no necesito liberarlo porque lo libera al destruir su pool
}

/*copies all the data from one buffer to another one and deletes the original*/
void replaceBuffer(VkBuffer originalBuffer, VkBuffer newBuffer, VkDeviceSize size) {
	//paso los contenidos
	copyBuffer(originalBuffer, 0, newBuffer, 0, size);//con VK_WHOLE_SIZE daba validation error diciendo que era un numero muy grande

	//destruyo lo anterior
	vkDestroyBuffer(logicalDevice, originalBuffer, nullptr);
}

VkDeviceMemory allocateDeviceMemory(std::vector<VkMemoryRequirements> memoryRequirements) {
	//hacemos que tenga espacio suficiente para todo lo que va tener y que tenga el tipo de memoria correcto
	VkDeviceSize totalMemorySize = calculateBuffersCombinedMemorySize(memoryRequirements);
	uint32_t allMemoryTypeBits = 0;
	for (auto& currMemRequirements : memoryRequirements) {
		allMemoryTypeBits |= currMemRequirements.memoryTypeBits;
	}

	//llenamos el struct
	VkMemoryAllocateInfo memoryAI{};
	memoryAI.allocationSize = totalMemorySize;
	memoryAI.memoryTypeIndex = findRigthMemoryType(allMemoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
	memoryAI.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;

	//lo creamos
	VkDeviceMemory returnMemory;
	if (vkAllocateMemory(logicalDevice, &memoryAI, nullptr, &returnMemory) != VK_SUCCESS) {
...
VkDeviceSize calculateBuffersCombinedMemorySize(std::vector<VkMemoryRequirements> memoryRequirements)
{
	...
	for (const auto& curReq : memoryRequirements) {
		totalSize += static_cast<VkDeviceSize>(curReq.alignment * ceil(curReq.size / curReq.alignment));//alignment por tama�o entre alignment
	}
...
{
	vkMapMemory(logicalDevice, memory, 0, calculateBuffersCombinedMemorySize(memoryRequirements), 0, &(mappedMemories->at(0)));

	uint32_t currMemOffset = 0;
	for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
		currMemOffset = static_cast<uint32_t>(memoryRequirements[i].alignment * ceil(currMemOffset / memoryRequirements[i].alignment));//modify the offset to fit with my alignment

		vkBindBufferMemory(logicalDevice, buffers[i], memory, currMemOffset);
		mappedMemories->at(i) = static_cast<void*>(static_cast<char*>(mappedMemories->at(0)) + currMemOffset);

		currMemOffset += static_cast<uint32_t>(memoryRequirements[i].size);//add my size to the offset for the next buffer
	}
}

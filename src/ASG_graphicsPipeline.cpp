...
asgPipeline::asgPipeline(VkFormat chosenSwapSurfaceFormat, VkFormat supportedDepthBufferFormat){
	...
	matrixBufferLayoutBinding.descriptorCount = 1;//se suele hacer de varios, por ejemplos para dar la matrix de cada transformaci�n de cada parte de un esqueleto
	...
	samplerLayoutBinding.descriptorCount = 50;//TODO este es un n�mero arbitrario de im�genes, si lo cambias, checa que no entre en conflicto con el tama�o de su pool
	...
	blendCreateInfo.logicOpEnable = VK_FALSE;//tambien podr�as blendear con bitwise ops, pero poner esto true hace el attachment false
	...
	colorAttachmentReference.layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;//el layout con el que tratar� ese attachment, en este caso como color buffer
	...
	//cada subpass necesita descripci�n
	...
	//tenemos 3 subpasses, la que creamos, la operaci�n antes y la operaci�n despu�s, vulkan tiene built-in dependencies que lidian con ellas pero hay que sincronizar la de la operaci�n despu�s
	VkSubpassDependency subpassDependency{};
	subpassDependency.srcSubpass = VK_SUBPASS_EXTERNAL; // VK_SUBPASS_EXTERNAL se refiere a la operaci�n antes o despu�s dependiendo de si est� en .srcSubpass o .dstSubpass
	subpassDependency.dstSubpass = 0;//indice de subpass, en este caso la de color
	//Esperamos a:
	subpassDependency.srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;//esperaremos a esta operaci�n
	subpassDependency.srcAccessMask = 0;//espec�ficamente a que 0 termine, osea a que 

	subpassDependency.dstStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;//esta operaci�n ser� la que espere
	subpassDependency.dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;//espec�ficamente esperaremos hasta que acabe y luego escribiremos

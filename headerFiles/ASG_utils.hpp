...
extern VkSurfaceKHR windowSurface;//lo usa una funci�n de la swapchain y getSelectedQueueFamilies(osea main), ademas de momento la crea en asInit y existe la posiblidad de que la necesite yo al usarlo(o no)
...
VkCommandBuffer createSingleUseCmdBuffer();//podr�a pedir la command pool, pero de momento uso la misma siempre y no se como sincronizaria esto con endSingleUseCommandBuffer(necesita usar el mismo pool)
...
//TODO para este podr�as a�adirle el offset que quieres que tenga como argumento y usarlo para todos los buffers o podr�as usarlo solo para staging y reducir argumentos, aunque eso te forzar�a a crear los uniform buffers de otra manera
